...
public class MapColoring extends CSPProblem {
    ...
    public class AllDiff extends Constraint {
        ...
        public boolean satisfied(Assignment assign) {
            boolean[] found = new boolean[boardSize + 1];
            for (Variable v : variables) {
                Integer val = (Integer) assign.getValue(v);
                if (val == null) {
                    return false;
                }
                found[val] = true;
    ...
    public List<Variable> variables() {
        if (tiles.isEmpty()) {
            ...
            tiles.add(new Map("Sumatra Utara"));
            tiles.add(new Map("Riau"));
            tiles.add(new Map("Sumatra Barat"));
            tiles.add(new Map("Jambi"));
            tiles.add(new Map("Bengkulu"));
            tiles.add(new Map("Sumatra Selatan"));
